// Credits: http://www.amphibian.com/blogstuff/collision.html
/*
...
Polygon.prototype.intersectsWith = function(other) {
	...
	for (side = 0; side < this.getNumberOfSides(); side++) { /* get the axis that we will project onto */
		if (side == 0) {
	...
	for (side = 0; side < other.getNumberOfSides(); side++) { /* get the axis that we will project onto */
		if (side == 0) {
...
Polygon.prototype.move = function(pNewCenter) {
	var xCenter = this.center.x;
	var yCenter = this.center.y;
	var xDelta	= pNewCenter.x - xCenter;
	var yDelta	= pNewCenter.y - yCenter;
	this.center.x = xCenter + xDelta;
	this.center.y = yCenter + yDelta;
	for (var i = 0; i < this.points.length; i++) {
		var x = this.points[i].x;
		var y = this.points[i].y;
		this.points[i].x = x + xDelta;
		this.points[i].y = y + yDelta;
	}
}	