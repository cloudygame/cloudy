...
(function(namespace) {
	...
	p.drawQuadraticJson = function(graphics, inJson) {
		...
		for (var i = 0; i < inJson.quadraticCurveTo.length; i++) {
			...
			var pCurr = new Point(this.shape.x + inJson["quadraticCurveTo"][i]["x"], this.shape.y + inJson["quadraticCurveTo"][i]["y"]);
			var pCurrRef = new Point(this.shape.x + inJson["quadraticCurveTo"][i]["ref_x"], this.shape.y + inJson["quadraticCurveTo"][i]["ref_y"]);
	...
	p.drawBoundingPolygon = function(colour) {
		...
		for (var ixSide = 0; ixSide < this.boundingPolygon.getNumberOfSides(); ixSide++) {
			if (ixSide == 0) {
				graphics.moveTo(this.boundingPolygon.center.x - this.shape.x, this.boundingPolygon.center.y - this.shape.y);
			} else {
				graphics.lineTo(this.boundingPolygon.points[ixSide].x - this.shape.x, this.boundingPolygon.points[ixSide].y - this.shape.y);
			}
		}
		graphics.closePath();
	}
